...
Release assets are being attached to a draft; final publication evidence will be recorded here. No macOS/Linux live microphone, permission-grant, or editor interaction was performed. Mac/Linux packages are initial previews pending interactive verification; Windows was checked on the development PC rather than a clean VM. Windows is unsigned; Mac is not Developer ID signed or notarized. See [Unix installation and build instructions](unix-installation.md).
